...
def _mark_standard_voice_backend() -> None:
    global _voice_backend
    _voice_backend = "send_input"


def send_wechat_key_combo_down(
    tokens: Sequence[str], *, _sender: Optional[RawSender] = None
) -> None:
    """Press a WeChat shortcut through the standard batched SendInput path.

    WeChat accepts the Windows input stream produced by SendInput more
    reliably than the legacy keybd_event path used by Doubao. Keeping this
    transport separate prevents a fix for one host from changing another
    host's shortcut semantics.
    """

    _mark_standard_voice_backend()
    send_key_combo_down(tokens, _sender=_sender)


def send_wechat_key_combo_up(
    tokens: Sequence[str], *, _sender: Optional[RawSender] = None
) -> None:
    """Release a WeChat shortcut through the standard SendInput path."""

    _mark_standard_voice_backend()
    send_key_combo_up(tokens, _sender=_sender)


def send_wechat_key_combo_tap(
    tokens: Sequence[str], *, _sender: Optional[RawSender] = None
) -> None:
    """Tap a WeChat shortcut through the standard SendInput path."""

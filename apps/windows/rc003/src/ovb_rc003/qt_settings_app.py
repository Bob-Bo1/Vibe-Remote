...
def _load_qt_classes() -> dict:
    ...
    class SettingsController(QObject):
        ...
        _integratedBridgeStatus = Signal(str)

        _TRIGGER_MODE_ORDER = tuple(key_mapping.VoiceTriggerMode)
        _VOICE_APP_ORDER = tuple(key_mapping.VOICE_APP_ORDER)
        ...
        def __init__(self, model: "ButtonMappingModel", parent=None) -> None:
            ...
            self._voice_app_id = key_mapping.voice_app_for_settings(
                self._config.get("voice_input_target"),
                self._hotkey_text,
                saved_trigger_mode,
            )
            self._voice_app_switching = False

            self._launch_status_text = settings_ui.LAUNCH_NOT_STARTED_TEXT
            self._connection_state = "idle"
            self._full_keys_status = (
                "全部按键已就绪"
                if self._config.get("hid_report_tap_enabled")
                else "连接时自动准备"
            )
            ...
            # path, while the end-user package gets the single-application
            # behavior without changing that fallback.
            if getattr(sys, "frozen", False):
                self._integrated_bridge = integrated_bridge.IntegratedBridgeRuntime(
                    on_status=self._integratedBridgeStatus.emit
                )
                self._integratedBridgeStatus.connect(
                    self._on_integrated_bridge_status
                )
        ...
        def _set_full_keys_status(self, text: str) -> None:
            if text != self._full_keys_status:
                self._full_keys_status = text
                self.fullKeysStatusChanged.emit()
        ...
        def _on_integrated_bridge_status(self, status: str) -> None:
            """Updates QML only on the Qt thread after the bridge worker
            reports a lifecycle change.
            """

            if status == integrated_bridge.STATUS_STARTING:
                self._set_connection_state("connecting")
                self._set_launch_status("正在当前 RemoteMicRC003.exe 内启动桥接…")
            elif status == integrated_bridge.STATUS_RUNNING:
                self._set_connection_state("waiting")
                self._set_launch_status(
                    "桥接已启动，正在等待遥控器连接。"
                )
            elif status == integrated_bridge.STATUS_DEVICE_CONNECTING:
                self._set_connection_state("connecting")
                self._set_launch_status("正在连接 RC003…")
            elif status == integrated_bridge.STATUS_DEVICE_CONNECTED:
                self._set_connection_state("connected")
                self._set_launch_status("RC003 已连接，按键和语音服务已就绪。")
            elif status == integrated_bridge.STATUS_DEVICE_DISCONNECTED:
                self._set_connection_state("waiting")
                self._set_launch_status("桥接已启动，正在等待遥控器连接。")
            elif status == integrated_bridge.STATUS_STOPPED:
                self._set_connection_state("idle")
                self._set_launch_status("桥接已停止。")
            elif status == integrated_bridge.STATUS_ALREADY_RUNNING:
                self._set_connection_state("waiting")
                self._set_launch_status(
                    "已有 RC003 桥接在运行，正在等待遥控器连接。"
                )
            elif status == integrated_bridge.STATUS_UNAVAILABLE:
                self._set_connection_state("error")
                self._set_launch_status(
                    "无法取得桥接单实例权限。请用管理员权限启动 RemoteMicRC003.exe 后重试。"
                )
            elif status == integrated_bridge.STATUS_FAILED:
                self._set_connection_state("error")
                self._set_launch_status(
                    "桥接启动失败。请点击“打开日志目录”查看 app.log。"
                )
        ...
        def _save(self) -> bool:
            ...
            try:
                new_config, new_bindings = settings_ui.build_save_model(
                    ...
                    voice_input_target=self._voice_app_id,
                )
        ...
        def _set_hotkey_text(self, value: str) -> None:
            ...
            if not self._voice_app_switching:
                current_mode = self._TRIGGER_MODE_ORDER[self._trigger_mode_index]
                inferred = key_mapping.voice_app_for_settings(
                    "", value, current_mode
                )
                if inferred != self._voice_app_id:
                    self._voice_app_id = inferred
                    self.voiceAppIndexChanged.emit()
        ...
        def _set_trigger_mode_index(self, value: int) -> None:
            if value != self._trigger_mode_index and 0 <= value < len(self._TRIGGER_MODE_ORDER):
                ...
                keep_custom_target = self._voice_app_id == key_mapping.VOICE_APP_CUSTOM
                self._voice_app_switching = keep_custom_target
                try:
                    self._set_hotkey_text(
                        settings_ui.voice_hotkey_for_trigger_mode(trigger_mode)
                    )
                finally:
                    self._voice_app_switching = False
                self.triggerModeIndexChanged.emit()
        ...
        def _get_voice_app_options(self) -> List[str]:
            return [key_mapping.VOICE_APP_LABELS[app_id] for app_id in self._VOICE_APP_ORDER]

        voiceAppOptions = Property(list, _get_voice_app_options, constant=True)

        def _get_voice_app_index(self) -> int:
            return self._VOICE_APP_ORDER.index(self._voice_app_id)

        def _set_voice_app_index(self, value: int) -> None:
            if not 0 <= value < len(self._VOICE_APP_ORDER):
                return
            app_id = self._VOICE_APP_ORDER[value]
            if app_id == self._voice_app_id:
                return
            self._voice_app_switching = True
            try:
                self._voice_app_id = app_id
                preset = key_mapping.voice_app_preset(app_id)
                if preset is not None:
                    preset_hotkey, preset_mode = preset
                    mode_index = self._TRIGGER_MODE_ORDER.index(preset_mode)
                    if mode_index != self._trigger_mode_index:
                        self._trigger_mode_index = mode_index
                        self.triggerModeIndexChanged.emit()
                    self._set_hotkey_text(preset_hotkey)
            finally:
                self._voice_app_switching = False
            self.voiceAppIndexChanged.emit()

        voiceAppIndex = Property(
            int,
            _get_voice_app_index,
            _set_voice_app_index,
            notify=voiceAppIndexChanged,
        )

        def _get_voice_app_description(self) -> str:
            return key_mapping.VOICE_APP_DESCRIPTIONS[self._voice_app_id]

        voiceAppDescription = Property(
            str, _get_voice_app_description, notify=voiceAppIndexChanged
        )

        def _get_voice_app_is_custom(self) -> bool:
            return self._voice_app_id == key_mapping.VOICE_APP_CUSTOM

        voiceAppIsCustom = Property(
            bool, _get_voice_app_is_custom, notify=voiceAppIndexChanged
        )

        def _get_voice_hotkey_display(self) -> str:
            labels = {
                key_mapping.VOICE_APP_WECHAT: "Ctrl+Win",
                key_mapping.VOICE_APP_DOUBAO: (
                    "右 Alt + Space"
                    if self._trigger_mode_index == self._TRIGGER_MODE_ORDER.index(
                        key_mapping.VoiceTriggerMode.TOGGLE
                    )
                    else "右 Alt"
                ),
                key_mapping.VOICE_APP_WINDOWS_DICTATION: "Win + H",
            }
            return labels.get(self._voice_app_id, self._hotkey_text)

        voiceHotkeyDisplay = Property(
            str, _get_voice_hotkey_display, notify=triggerModeIndexChanged
        )
        ...
        def _get_full_keys_status(self) -> str:
            return self._full_keys_status

        fullKeysStatus = Property(
            str, _get_full_keys_status, notify=fullKeysStatusChanged
        )
        ...
        def saveAndLaunch(self) -> None:
            ...
            # Full-key support is an implementation detail of the RC003
            # connection flow. The first connection may ask for one visible
            # consent because it changes a narrowly-scoped Defender setting;
            # the user never has to understand or configure a "full mode".
            self._prepare_full_keys_for_connection()
            self._launch_bridge()

        def _prepare_full_keys_for_connection(self) -> None:
            if self._config.get("hid_report_tap_enabled"):
                self._set_full_keys_status("全部按键已就绪")
                return
            self._set_full_keys_status("正在准备全部按键…")
            try:
                result = full_keys_setup.enable_full_keys(confirm=True)
            except Exception as exc:  # noqa: BLE001 - keep basic connection usable
                self._set_full_keys_status(f"全部按键准备失败，基础按键仍可用：{exc}")
                return
            if result.ok:
                try:
                ...
                except Exception as exc:  # noqa: BLE001 - report stale state clearly
                    self._set_full_keys_status(f"全部按键已准备，但配置回读失败：{exc}")
                else:
                    self._set_full_keys_status("全部按键已就绪")
        ...
        @Slot(result=bool)
        def prepareFullKeys(self) -> bool:
            """Manually retry the same first-use preparation flow."""

            if self._selected_device_id() != device_catalog.RC003_ID:
                self._set_full_keys_status("当前设备不需要 RC003 特殊按键支持")
                return False
            self._prepare_full_keys_for_connection()
            return bool(self._config.get("hid_report_tap_enabled"))

        def _launch_bridge(self) -> None:
            if self._integrated_bridge is not None:
                # The packaged exe owns the bridge thread itself. Keeping
                # this in the same controller means the visible settings
                # window and the running bridge share one saved configuration.
                self._set_connection_state("connecting")
                self._set_launch_status("正在当前 RemoteMicRC003.exe 内启动桥接…")
                if not self._integrated_bridge.start():
                    self._set_connection_state("error")
                    self._set_launch_status(
                        "桥接未能干净重启。请关闭旧的 RemoteMicRC003.exe 后重试。"
                    )
                return

            # Source/dev runs retain the existing child-process seam.  It is
            # also the path used by the legacy administrator CMD launcher,
            # so this fallback stays compatible with older test/build setups.
            self._set_connection_state("connecting")
        ...
        @Slot()
        def shutdownBridge(self) -> None:
            """Stops the in-process bridge before the Qt runtime is torn down."""

            if self._integrated_bridge is not None:
                self._integrated_bridge.stop()
        ...
        def restoreDefaults(self) -> None:
            ...
            self._set_voice_app_index(
                self._VOICE_APP_ORDER.index(defaults.voice_input_target)
            )
            self._set_error_message("")
            self._set_status_message(
...
def run_settings_window() -> int:
    ...
    # Basic supports our shared controls consistently on Windows and in previews.
    QQuickStyle.setStyle("Basic")
    app = QGuiApplication.instance() or QGuiApplication(sys.argv)
    from PySide6.QtGui import QIcon
    app.setWindowIcon(QIcon(str(_qml_directory() / "assets" / "app-icon.ico")))
    ...
    try:
        ...
        # The packaged settings process also owns the integrated bridge
        # worker.  Stop it while Qt and Python objects are still alive, before
        # the diagnostics/Qt teardown below begins.
        controller.shutdownBridge()

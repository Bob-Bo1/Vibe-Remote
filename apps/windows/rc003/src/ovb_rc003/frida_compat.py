...
class RC003HidReportTap:
    ...
    def _status(self, message: str) -> None:
        """Report tap state to the owning app when one is available.

        The packaged GUI has no console, so the old ``print``-only diagnostics
        made an injection failure look exactly like a working tap.  Keep the
        console output for source/debug runs while also giving the app a
        logging seam for persistent diagnostics.
        """
        ...
        if handler is not None:
            try:
                ...
                # Diagnostics must never take down the HID tap thread.
                pass
        if sys.stdout is not None:
            try:
                print(f"RC003 HID TAP {message}", flush=True)
            except Exception:
                pass
    ...
    def _handle_ioctl_output(self, data: bytes) -> None:
        ...
        self._status(f"{' '.join(changes)} raw={data.hex()}")

    def _run(self) -> None:
        """Keep the tap alive and make failures visible to the app log."""
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self._status(f"fatal_retry {type(exc).__name__}: {exc}")
        ...
        while not self.stop_event.is_set():
            ...
            if pid is None:
                ...
                if now - self._last_wait_log >= 30.0:
                    ...
                    self._status("waiting_for_rc003_host")
                self.stop_event.wait(self.retry_delay)
                continue
            ...
            try:
                ...
                if injection_attempted_pid is None:
                    try:
                        ...
                        self._status(
                            f"injection_retry {type(exc).__name__}: {exc}"
                        )
                        self.stop_event.wait(self.retry_delay)
                        continue
                try:
                ...
                try:
                    self._status(f"ATTACHED pid={pid} awaiting_io=true")
                    buffer = b""
                    ...
                    while not self.stop_event.is_set():
                        if frida_hid_tap_runtime.find_rc003_hidogatt_host_pid() != pid:
                            self._status(f"HOST_CHANGED old_pid={pid}")
                            injection_attempted_pid = None
                            break
                        try:
                        ...
                        if chunk:
                            ...
                            while b"\n" in buffer:
                                ...
                                if kind in {"heartbeat", "ready"}:
                                    ...
                                    self._status(
                                        f"hook_error={message.get('message')}"
                                    )
                        now = time.monotonic()
                        if now - last_heartbeat >= self.heartbeat_timeout:
                            self._status(
                                f"UNHEALTHY pid={pid} reason=agent_heartbeat_stale"
                            )
                            break
                        if io_verified and not announced_ready:
                            announced_ready = True
                            self._status(f"READY pid={pid} io_verified=true")
                finally:
                    try:
                        client.close()
                    except OSError:
    ...
    def start(self) -> bool:
        if not self.enabled:
            ...
            return False
        if not self.dependency_available:
            self._status("unavailable verified_gadget_not_installed")
            return False
        if self.thread is not None and self.thread.is_alive():
            return True

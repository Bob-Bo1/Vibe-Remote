...
def default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # The optional Frida/WUDFHost injection path is disabled by default.
        # Ordinary RC003 buttons use the device-scoped Raw Input listener;
        # the injection path is only needed for usages Windows drops.
        "hid_report_tap_enabled": False,
        "voice_shortcut_enabled": True,
        # New installations use the verified WeChat hold-to-speak flow.
        # Older configs without this field are inferred below from their
        # saved shortcut, so an existing Doubao setup is preserved.
        "voice_input_target": key_mapping.VOICE_APP_DEFAULT,
...
def load_config(path: Path) -> Dict[str, Any]:
    ...
    if "voice_input_target" not in stored:
        config["voice_input_target"] = key_mapping.infer_voice_app_id(
            config.get("voice_hotkey", ""),
            _voice_trigger_mode_or_none(config.get("voice_trigger_mode")),
        )
...
def _normalize_voice_hotkey(config: Dict[str, Any]) -> None:
    ...
    target = key_mapping.normalize_voice_app_id(config.get("voice_input_target"))
    ...
    # ``lalt`` was an invalid recording of the RC003 F5 leak. Repair it even
    # when the file predates the voice-app target field.
    if mode == key_mapping.VoiceTriggerMode.HOLD and current == "lalt":
        config["voice_hotkey"] = "ralt"
        return

    # Preserve the previous Doubao defaults when loading/updating an old
    # config. The generic trigger-mode defaults now belong to the WeChat
    # preset, so routing these legacy values through them would unexpectedly
    # change an existing user's input method.
    if target == key_mapping.VOICE_APP_DOUBAO:
        if mode == key_mapping.VoiceTriggerMode.HOLD and current == "ralt+space":
            config["voice_hotkey"] = "ralt"
        elif mode == key_mapping.VoiceTriggerMode.TOGGLE and current == "ralt":
            config["voice_hotkey"] = "ralt+space"
        return
    if target == key_mapping.VOICE_APP_CUSTOM:
        return
...
def _voice_trigger_mode_or_none(value: object):
    from . import key_mapping

    try:
        return key_mapping.VoiceTriggerMode(value)
    except ValueError:
        return None

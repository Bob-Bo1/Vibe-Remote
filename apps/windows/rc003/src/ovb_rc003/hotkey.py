...
# The fresh-install default follows the user-facing voice-app preset. Users
# can switch to Doubao, Windows dictation, or a custom shortcut in settings.
# Existing config files keep their saved value because config.load_config()
# merges them over default_config() and infers old presets when needed.
DEFAULT_VOICE_HOTKEY = HotkeySpec.parse(
    key_mapping.voice_app_preset(key_mapping.VOICE_APP_DEFAULT)[0]
)

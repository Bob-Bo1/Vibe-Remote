...
# RC003 firmware can report AUDIO_STARTED a few tens of milliseconds before
# the keyboard-class F5 edge reaches the low-level hook.  Give that physical
# edge a short opportunity to win, so the WeChat shortcut is sent from the
# earliest path instead of waiting behind an audio-start fallback.
_VOICE_AUDIO_START_GRACE_SECONDS = 0.12
# AUDIO_STOP can arrive just before the physical F5-up edge.  Hold the host
# shortcut for only this short safety window so the physical release can own
# the key-up (and a missing F5-up still cannot leave Win held forever).
_VOICE_AUDIO_STOP_RELEASE_GRACE_SECONDS = 0.25
_WINDOWS_WASAPI_HOST_API = "Windows WASAPI"
_WINDOWS_DIRECTSOUND_HOST_API = "Windows DirectSound"

DEVICE_CONNECTING_STATUS = "device_connecting"
DEVICE_CONNECTED_STATUS = "device_connected"
DEVICE_DISCONNECTED_STATUS = "device_disconnected"
...
class RC003App:
    def __init__(self, on_status: Optional[Callable[[str], None]] = None) -> None:
        ...
        self._voice_audio_start_timer: Optional[threading.Timer] = None
        self._voice_audio_stop_release_timer: Optional[threading.Timer] = None
        # HOLD-mode sessions are closed by the physical mic-up edge whenever
        # that edge is available.  This prevents a late AUDIO_STARTED from
        # keeping the previous Ctrl/Win pair logically active.
        self._voice_physical_hold_active = False
        self._voice_ignore_late_control = False
        self._voice_legacy_f5_seen = False
    ...
    async def _connect_once(self) -> None:
        self._emit_status(DEVICE_CONNECTING_STATUS)
        # RC003's ordinary buttons are delivered through Windows HID and do
        # not depend on a successful BLE/ATVV handshake.  Start both button
        # paths first so Return/volume/navigation remain usable while BLE is
        # still discovering, reconnecting, or temporarily unavailable.
        self._logger.info(
            "startup: starting RC003 button paths before BLE discovery/handshake"
        )
        ...
        # Open the selected output before the user presses the mic key.  The
        # PortAudio device negotiation can take hundreds of milliseconds on
        # Windows; WeChat's Ctrl+Win voice input starts on the held key edge,
        # so opening it lazily would make the shortcut arrive after the first
        # part of the remote audio (or after a very short stream has ended).
        self._prewarm_voice_playback()
        self._emit_status(DEVICE_CONNECTED_STATUS)

    def _emit_status(self, status: str) -> None:
        try:
            self._on_status(status)
        except Exception:  # noqa: BLE001 - status reporting must not kill bridge
            self._logger.exception("RC003 status callback failed")
    ...
    def _start_hid_report_tap(self) -> None:
        ...
        if not bool(self._config.get("hid_report_tap_enabled", False)):
            self._logger.info(
                "startup: RC003 HID report tap disabled; using Raw Input only"
            )
            return

        tap = frida_compat.RC003HidReportTap(
            self._on_direct_hid_report,
            status_handler=lambda message: self._logger.info("hid tap: %s", message),
        )
        try:
            if tap.start():
                self._hid_report_tap = tap
                self._logger.info(
                    "startup: RC003 optional HID report tap started; "
                    "waiting for ATTACHED/READY"
                )
            else:
                self._logger.info(
    ...
    def _on_direct_hid_report(self, report_id: int, payload: bytes) -> None:
        ...
        is handled here too: it is the earliest reliable edge for a held
        WeChat Ctrl+Win shortcut, while the later F5/ATVV events are merely
        deduplicated confirmations of the same voice session.
        """
        ...
        for usage in sorted(pressed):
            ...
            if button == "mic":
                # The HID report for Mic is delivered on a slower, blocking
                # side channel on this RC003.  The keyboard-class F5 edge is
                # the reliable low-latency trigger; ATVV AUDIO_STARTED keeps
                # the fallback when F5 is unavailable.  Do not let this
                # report contend for the voice lock and delay Ctrl+Win.
                continue
            self._logger.info(
        ...
        for usage in sorted(released):
            button = frida_compat.TAP_USAGE_TO_BUTTON[usage]
            if button == "mic":
                # The tap's release edge is the earliest reliable indication
                # that the user has let go of the remote.  We intentionally do
                # not use its press edge (the RC003 tap can deliver that edge
                # on a blocking side channel), but ignoring the release too
                # leaves the Ctrl+Win shortcut held until the 250 ms safety
                # timer or a late F5-up.  That delayed release makes WeChat's
                # voice text commit arrive one character at a time and can
                # overlap the next press.  Let the normal voice state machine
                # close the host shortcut immediately; later F5/ATVV releases
                # are idempotently ignored there.
                self._logger.info(
                    "RC003 direct HID usage up: 0x%04x -> mic",
                    usage,
                )
                self._on_button_event("mic", False)
                continue
            self._logger.info(
    ...
    def _prewarm_voice_playback(self) -> None:
        ...
        if self._open_playback_for_new_session():
            ...
            self._logger.info(
                "startup: voice playback prewarm unavailable; will retry on mic press"
            )
    ...
    async def _cleanup_once(self) -> None:
        ...
        try:
            with self._voice_trigger_lock:
                self._cancel_voice_audio_start_timer_locked()
                self._cancel_voice_audio_stop_release_timer_locked()
                self._voice_audio_start_fallback_pending = False
                self._voice_audio_started_waiting_for_legacy_f5 = False
                self._voice_raw_input_trigger_pending = False
                self._voice_physical_hold_active = False
        ...
        self._emit_status(DEVICE_DISCONNECTED_STATUS)
    ...
    def _on_legacy_key_event(self, vk_code: int, is_pressed: bool) -> None:
        ...
        if vk_code == 0x74:
            if is_pressed:
                ...
                self._voice_legacy_f5_seen = True
                # A new physical press starts a fresh control-event window.
                # Any ATVV MIC_BUTTON that arrives after the previous F5-up
                # belongs to the old, delayed device session and must not
                # reopen WeChat on its own.
                self._voice_ignore_late_control = False
                self._logger.info(
    ...
    def _cancel_voice_audio_start_timer_locked(self) -> None:
        """Cancel a delayed AUDIO_STARTED fallback while holding the voice lock."""

        timer = self._voice_audio_start_timer
        self._voice_audio_start_timer = None
        if timer is not None:
            timer.cancel()

    def _cancel_voice_audio_stop_release_timer_locked(self) -> None:
        """Cancel the physical-release safety timer while holding the voice lock."""

        timer = self._voice_audio_stop_release_timer
        self._voice_audio_stop_release_timer = None
        if timer is not None:
            timer.cancel()

    def _schedule_voice_audio_stop_release_timer_locked(self) -> None:
        """Release a held shortcut if the physical F5-up edge is lost."""

        self._cancel_voice_audio_stop_release_timer_locked()
        timer = threading.Timer(
            _VOICE_AUDIO_STOP_RELEASE_GRACE_SECONDS,
            self._run_voice_audio_stop_release_fallback,
        )
        timer.daemon = True
        self._voice_audio_stop_release_timer = timer
        timer.start()

    def _run_voice_audio_stop_release_fallback(self) -> None:
        """Close a HOLD session after AUDIO_STOP when F5-up never arrives."""

        action = None
        transformed_session = False
        with self._voice_trigger_lock:
            self._voice_audio_stop_release_timer = None
            if not self._voice_physical_hold_active or not self._voice.active:
                return
            self._voice_physical_hold_active = False
            action = self._voice.on_audio_stopped()
            transformed_session = self._voice_legacy_transform_session
            self._logger.info(
                "voice physical mic-up not observed; releasing host shortcut after %.0f ms",
                _VOICE_AUDIO_STOP_RELEASE_GRACE_SECONDS * 1000.0,
            )
        self._deliver_voice_close_action(action, "AUDIO_STOP safety release")
        if transformed_session:
            self._voice_legacy_transform_session = False

    def _deliver_voice_close_action(
        self,
        action: Optional[voice_controller.VoiceHostAction],
        reason: str,
    ) -> None:
        """Deliver a closing action after the state lock has been released."""
        ...
        self._logger.info(
            "voice closing action failed to fully deliver (%s); requesting reconnect",
            reason,
        )
        self._supervisor.request_reconnect()

    def _schedule_voice_audio_start_fallback_locked(self) -> None:
        """Let physical F5/HID edges win before using AUDIO_STARTED fallback."""

        self._cancel_voice_audio_start_timer_locked()
        timer = threading.Timer(
            _VOICE_AUDIO_START_GRACE_SECONDS,
            self._run_voice_audio_start_fallback,
        )
        timer.daemon = True
        self._voice_audio_start_timer = timer
        timer.start()

    def _run_voice_audio_start_fallback(self) -> None:
        """Trigger voice only when no physical edge arrived during the grace."""

        with self._voice_trigger_lock:
            self._voice_audio_start_timer = None
            if not self._voice_audio_start_fallback_pending:
                return
            self._voice_audio_start_fallback_pending = False
            if self._voice.active:
                return
            self._logger.info("voice audio-start fallback fired after physical-edge grace")
            self._handle_mic_button_pressed(send_device_open=False)
            # Keep the latch until the matching ATVV MIC_BUTTON event arrives,
            # preventing that late confirmation from sending a second shortcut.
            ...
            if not is_pressed:
                ...
                with self._voice_trigger_lock:
                    ...
                    if self._voice.trigger_mode == key_mapping.VoiceTriggerMode.HOLD:
                        self._voice_ignore_late_control = True
                    if (
                        self._voice.trigger_mode == key_mapping.VoiceTriggerMode.HOLD
                        and self._voice_physical_hold_active
                        ...
                        transformed_session = self._voice_legacy_transform_session
                        self._logger.info(
                            "voice physical mic-up received; releasing host shortcut"
                        )
                # Keep the low-level hook free of the voice state lock while
                # the Win32 key-up is delivered.  This also lets a following
                # device event observe the already-closed logical session.
                self._deliver_voice_close_action(action, "physical mic-up")
                if transformed_session:
                    self._voice_legacy_transform_session = False
                return
            with self._voice_trigger_lock:
                if self._voice.active:
                    if self._voice.trigger_mode == key_mapping.VoiceTriggerMode.HOLD:
                        # AUDIO_STARTED fallback may have opened the session
                        # before F5 reached the hook.  Treat this F5-down as
                        # the physical owner of the hold, while suppressing a
                        # second Ctrl+Win delivery.
                        self._voice_physical_hold_active = True
                    self._logger.info(
                ...
                if not self._voice.active:
                    self._voice_raw_input_trigger_pending = False
                elif self._voice.trigger_mode == key_mapping.VoiceTriggerMode.HOLD:
                    self._voice_physical_hold_active = True
            return
        if is_pressed:
    ...
    def _on_button_trigger(
        ...
        try:
            ...
            return
        self._logger.info(
            "RC003 button action: button=%s trigger=%s action=%s",
            button_id,
            trigger.value,
            action.kind.value,
        )
    ...
    def _on_control_event(self, event: object) -> None:
        if isinstance(event, CapsReceived):
            ...
            with self._voice_trigger_lock:
                if self._voice_ignore_late_control:
                    self._logger.info(
                        "voice mic trigger ignored: late ATVV event after physical mic-up"
                    )
                    ...
                    fallback_waiting = self._voice_audio_start_timer is not None
                    self._cancel_voice_audio_start_timer_locked()
                    self._voice_audio_start_fallback_pending = False
                    if fallback_waiting:
                        self._logger.info(
                            "voice mic trigger received during AUDIO_STARTED grace"
                        )
                        self._handle_mic_button_pressed(send_device_open=False)
                        self._voice_audio_start_fallback_pending = self._voice.active
                    else:
            ...
            with self._voice_trigger_lock:
                ...
                if not self._voice.active:
                    if self._legacy_voice_transform_enabled():
                        ...
                        self._voice_audio_started_waiting_for_legacy_f5 = True
                        self._open_playback_for_new_session()
                    elif self._voice_legacy_f5_seen:
                        # This machine has already shown a reliable F5 edge.
                        # Keep waiting for that physical edge instead of
                        # letting a delayed AUDIO_STARTED reopen a session
                        # after the user has released the remote.
                        self._logger.info(
                            "voice audio started; waiting for known physical F5 edge"
                        )
                        self._voice_audio_started_waiting_for_legacy_f5 = True
                        self._open_playback_for_new_session()
                    else:
                        self._logger.info(
                            "voice audio start waiting briefly for physical mic edge"
                        )
                        self._voice_audio_start_fallback_pending = True
                        self._schedule_voice_audio_start_fallback_locked()
                else:
                    self._cancel_voice_audio_start_timer_locked()
                    self._voice_audio_start_fallback_pending = False
            ...
            with self._voice_trigger_lock:
                ...
                self._voice_audio_started_waiting_for_legacy_f5 = False
                self._voice_raw_input_trigger_pending = False
                deferred_to_physical_up = (
                    self._voice.trigger_mode == key_mapping.VoiceTriggerMode.HOLD
                    and self._voice_physical_hold_active
                    and self._voice.active
                )
                if deferred_to_physical_up:
                    self._logger.info(
                        "voice AUDIO_STOP observed; waiting for physical mic-up before key release"
                    )
                    self._schedule_voice_audio_stop_release_timer_locked()
                else:
                    action = self._voice.on_audio_stopped()
                    transformed_session = self._voice_legacy_transform_session
            self._deliver_voice_close_action(action, "AUDIO_STOP")
            if transformed_session:
                self._voice_legacy_transform_session = False
    ...
    def _apply_voice_action(self, action: voice_controller.VoiceHostAction) -> bool:
        ...
        if self._voice_legacy_transform_session:
            ...
            return True
        try:
            # WeChat's Ctrl+Win global voice shortcut is more stable with the
            # standard batched SendInput stream. Doubao keeps the marked
            # keybd_event path because its own hook needs the physicalized
            # virtual-key shape.
            wechat_shortcut = self._voice_hotkey.serialize() in {
                "lctrl+win",
                "lctrl+lwin",
            }
            if action == voice_controller.VoiceHostAction.TAP:
                sender = (
                    win32_input.send_wechat_key_combo_tap
                    if wechat_shortcut
                    else win32_input.send_voice_key_combo_tap
                )
                sender(tokens)
            elif action == voice_controller.VoiceHostAction.KEY_DOWN:
                sender = (
                    win32_input.send_wechat_key_combo_down
                    if wechat_shortcut
                    else win32_input.send_voice_key_combo_down
                )
                sender(tokens)
            else:
                sender = (
                    win32_input.send_wechat_key_combo_up
                    if wechat_shortcut
                    else win32_input.send_voice_key_combo_up
                )
                sender(tokens)
            self._logger.info(
                "voice hotkey delivered via %s: action=%s keys=%s",
                win32_input.voice_backend_name(),
                action.value,
                "+".join(tokens),
            )
    ...
    def _open_playback_for_new_session(self) -> bool:
        ...
        try:
            ...
            selected_endpoint = audio_output.resolve_selected_endpoint(
                endpoints, endpoint_name, endpoint_host_api
            )
            # DirectSound exposes VB-CABLE with a comparatively large shared
            # buffer (the current machine reports about 120 ms before any
            # recognizer-side buffering).  WASAPI exposes the same endpoint
            # at roughly 2 ms.  Keep the user's endpoint selection strict,
            # but transparently choose the lower-latency view when it is the
            # standard CABLE Input and the selected DirectSound view has a
            # matching WASAPI view.
            if (
                selected_endpoint.host_api == _WINDOWS_DIRECTSOUND_HOST_API
                and audio_output.is_cable_input_endpoint(selected_endpoint.name)
            ):
                wasapi_matches = [
                    endpoint
                    for endpoint in endpoints
                    if endpoint.name == selected_endpoint.name
                    and endpoint.host_api == _WINDOWS_WASAPI_HOST_API
                ]
                if len(wasapi_matches) == 1:
                    selected_endpoint = wasapi_matches[0]
                    self._logger.info(
                        "voice playback switched to low-latency host API: %s",
                        _WINDOWS_WASAPI_HOST_API,
                    )
            sink = audio_playback.EndpointPlaybackSink(
                selected_endpoint.name, selected_endpoint.host_api
            )
            sink.open()
            self._playback = sink
            self._logger.info(
                "voice playback opened: host_api=%s sample_rate=%s channels=%s",
                selected_endpoint.host_api or "unspecified",

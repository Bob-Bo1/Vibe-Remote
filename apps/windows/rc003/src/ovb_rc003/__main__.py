...
- ``--enable-full-keys``  request consent, configure the verified HID tap,
                          then start the bridge
- ``--disable-full-keys`` restore the default HID mode and remove the exact
                           Defender runtime-directory exclusion
...
def _print_help() -> None:
    ...
    print("Return and volume HID usages verified on one real RC003 setup; see README.md for scope.")
    print()
    print("Usage:")
    ...
    print("  python -m ovb_rc003 --enable-full-keys  enable full RC003 keys and start the bridge")
    print("  python -m ovb_rc003 --disable-full-keys disable full RC003 keys")
...
def main() -> None:
    ...
    if "--enable-full-keys" in args:
        from . import full_keys_setup

        result = full_keys_setup.enable_full_keys()
        if not result.ok:
            full_keys_setup.show_result(result)
            raise SystemExit(0 if result.cancelled else 1)
        full_keys_setup.show_result(result)
        _run_bridge()
        return
    if "--disable-full-keys" in args:
        from . import full_keys_setup

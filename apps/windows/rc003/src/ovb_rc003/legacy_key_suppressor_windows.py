...
# downstream hooks and is never used for ordinary input. Every key in a
# configured voice shortcut carries this marker, including Ctrl+Win for
# WeChat's global voice input.
                    ...
                    if target is not None:
                        ...
                        if self._on_key_event is not None:
                            try:
                                ...
                                pass
                        return 1
            # F5 is the dedicated RC003 voice edge.  It is already handled
            # by ``handle_key_event`` below, so it must not enter the
            # Raw-Input correlation wait.  Waiting here for every F5
            # auto-repeat blocks the global hook for ~60 ms per repeat and
            # can delay the host Ctrl+Win release into the next press.
            if (
                not (int(event.flags) & LLKHF_INJECTED)
                and int(event.vkCode) not in self._suppress_vk_codes

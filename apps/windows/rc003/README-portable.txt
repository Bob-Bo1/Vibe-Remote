...
4. 连接设备后即可使用遥控器语音。默认不启用 VB-CABLE，微信或豆包会使用 Windows
   当前的默认麦克风，也就是笔记本内置麦克风等系统录音设备。
5. VB-CABLE 是可选的遥控器语音增强通道。需要把遥控器麦克风声音送入微信、豆包
   或 Windows 听写时，再按页面提示检查并安装 CABLE Input/Output。缺少 VB-CABLE 时，
   点击“安装 VB-CABLE”；安装程序打开后，页面会显示“安装后需要重启电脑”，请完成
   官方安装、重启 Windows，再回来点击“重新检测”。也可以从 VB-Audio 官方页面手动
   下载：https://vb-audio.com/Cable/。检测通过后点击“使用 CABLE Input”；语音软件
   的麦克风输入选择 CABLE Output，Remote Mic 的“语音输出设备”选择 CABLE Input。
6. 新安装默认使用微信电脑端：按住 Ctrl+Win 说话，松开结束。切换到豆包时，选择
   豆包预设；需要切换模式时使用 ralt+space，需要按住模式时使用 ralt。Windows
   系统听写使用 Win+H。未启用 CABLE 时，以上语音软件继续使用 Windows 默认麦克风。
7. 选择语音软件后，按页面提示测试普通按键和语音。右上角显示“已连接”才表示
...
设置单击、双击和长按动作；长动作列表可以用滚轮继续查看。录入后卡片会立即显示新按键；点击底部保存才会把修改持久化。
点击“录”后，请在弹出的窗口中按一次本地实体键盘；窗口会优先使用系统全局监听，
同时保留当前窗口的键盘回传兜底，单键和组合键都可以录入。

...
**一键随包安装（XRBM-031，可选）**：打开"设置 → 连接与配置"页，在"准备语音通道"
步骤的"VB-CABLE 虚拟音频驱动"卡片中查看 CABLE Input/CABLE Output 两个端点当前是
...
- 提供“连接与配置”“按键映射”“诊断”三个设置页面；连接与配置页按顺序带用户完成连接、语音通道、语音软件选择和测试，诊断页只保留运行状态和日志入口。CABLE 端点检测结果直接显示在语音通道步骤中，不会把进程存活伪装成硬件验收通过。

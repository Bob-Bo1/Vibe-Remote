...
hiddenimports = [
    ...
    "ovb_rc003.integrated_bridge",  # packaged settings + bridge runtime
...
# The environment used for this candidate also contains Poppler.  Its
# ``icuuc.dll`` is collected automatically, but it exports ICU 78 symbols
# (names ending in ``_78``).  Qt6Core imports the Windows ICU ABI with
# un-suffixed names, so keeping Poppler's DLL beside Qt6Core makes every
# PySide6 import fail with WinError 127.  Windows supplies the compatible
# system ICU DLL; remove only this conflicting collected binary.
a.binaries = [
...
exe = EXE(
    ...
    upx=False,
    console=False,
    icon=str(QML_SOURCE_DIR / "assets" / "app-icon.ico"),
    # The Frida HID tap must inject its verified Gadget into the RC003
    # WUDFHost.  Request elevation at the executable boundary so a user who
    # double-clicks the settings exe gets the administrator token required by
    # the integrated bridge runtime and the legacy CMD ``--bridge`` fallback.
    # The CMD launcher itself remains unchanged.
    uac_admin=True,
)

...
class WeChatKeyComboTests(unittest.TestCase):
    def test_down_uses_one_batched_sendinput_call(self):
        sender = RecordingSender()

        win32_input.send_wechat_key_combo_down(
            ("lctrl", "lwin"), _sender=sender
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                (win32_input.win32_keys.VK_CODES["lwin"], False),
            ],
        )

    def test_up_uses_one_batched_sendinput_call_in_reverse_order(self):
        sender = RecordingSender()
        ...
        self.assertEqual(
            sender.calls[0],
            [
                (win32_input.win32_keys.VK_CODES["lwin"], True),
                (win32_input.win32_keys.VK_CODES["lctrl"], True),
            ],
        )

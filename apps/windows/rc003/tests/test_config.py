...
class DefaultConfigPrivacyTests(unittest.TestCase):
    def test_default_config_preserves_existing_users_on_rc003(self):
        ...
        self.assertEqual(config.default_config()["voice_input_target"], key_mapping.VOICE_APP_WECHAT)
        self.assertEqual(config.default_config()["voice_hotkey"], "lctrl+lwin")
    ...
    def test_hid_report_tap_is_opt_in(self):
        self.assertFalse(config.default_config()["hid_report_tap_enabled"])
    ...
    def test_save_repairs_a_stale_hold_right_alt_space_pair(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            data.update({
                "voice_input_target": key_mapping.VOICE_APP_DOUBAO,
    ...
    def test_load_preserves_recorded_left_ctrl_win_and_repairs_hold_mode(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "config.json"
            path.write_text(
                json.dumps(

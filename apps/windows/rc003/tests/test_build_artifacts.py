...
_PORTABLE_README_PATH = _RC003_ROOT / "README-portable.txt"
...
class WindowsCiWorkflowTests(unittest.TestCase):
    ...
    def test_does_not_compile_or_upload_an_installer(self):
        effective = _strip_hash_comments(self.text)
        self.assertNotIn("ISCC.exe", effective)
        self.assertNotIn("innosetup", effective.lower())
        self.assertNotIn("dist/installer", effective)
        self.assertNotIn("RemoteMicRC003Setup-", effective)
    ...
    def test_exactly_one_upload_step_runs_after_every_required_gate(self):
        # Exactly one upload-artifact step must exist, after the deterministic
        # portable packaging step.
    ...
    def test_portable_zip_stages_license_copyright_notices_attribution_and_readme(self):
        ...
        self.assertIn(
            'Copy-Item -Path "README-portable.txt" -Destination (Join-Path $stagingDir "README.txt")',
            self.text,
        )
    ...
    def test_successful_archive_continues_into_release_staging_and_preflight(self):
        # The try/catch's happy path must fall through to release-directory
        # staging and the hard preflight in the same step.
        step = self._package_step_text()
        ...
        self.assertLess(try_block_end, release_dir_index)
        self.assertLess(release_dir_index, preflight_index)

    def test_portable_staging_directory_is_a_single_short_top_level_folder(self):
        ...
        # short folder, not loose files scattered at the archive root.
        self.assertIn('$stagingName = "RemoteMicRC003"', self.text)
        self.assertIn('$stagingDir = "dist/portable/$stagingName"', self.text)

    def _package_step_text(self):
        start = self.text.index(
            "- name: Package deterministic unsigned portable release"
        )
        end = self.text.index(
            "- name: Upload portable distribution artifacts", start
        )
        return self.text[start:end]

    def _upload_step_text(self):
        return self.text[
            self.text.index("- name: Upload portable distribution artifacts") :
        ]

    def test_release_directory_is_staged_from_the_final_portable_zip(self):
        # The release directory is a CLEAN copy target, so a stale file left
        # over from a previous run can never leak into an upload.
        step = self._package_step_text()
        self.assertIn(
    ...
    def test_release_manifest_is_generated_from_the_staged_copies_not_the_originals(self):
        # The manifest must hash the file actually sitting in dist/release,
        # not the original under dist/portable, so a copy failure is
        # reflected in the preflight below.
        step = self._package_step_text()
        self.assertIn(
    ...
    def test_preflight_hard_checks_exactly_two_files_with_the_expected_names(self):
        step = self._package_step_text()
        self.assertIn("$releaseFiles.Count -ne 2", step)
        self.assertIn(
    ...
    def test_preflight_hard_checks_exactly_one_well_formed_manifest_line(self):
        step = self._package_step_text()
        self.assertIn("$manifestLines.Count -ne 1", step)
        self.assertIn(
    ...
    def test_disclosure_still_denies_real_hardware_and_installer_execution(self):
        ...
        self.assertNotIn("run the compiled installer", lower)
...
class UserFacingDocumentationContractTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.both = (self.readme_text, self.portable_readme_text)
    ...
    def test_exact_cable_routing_direction_is_preserved_in_both_docs(self):
        ...
        self.assertIn("语音输出设备", self.portable_readme_text)
        self.assertIn(
            "CABLE Input", self.portable_readme_text.split("语音输出设备")[1][:120]
        )
...
class PrereleaseDownloadInstructionsContractTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.project_text = _PYPROJECT_PATH.read_text(encoding="utf-8")
    ...
    def test_asset_name_patterns_match_the_ci_workflows_actual_output_names(self):
        ...
        # above, and the package version metadata below.
        self.assertIn(
    ...
    def test_documents_the_release_tag_vs_internal_build_version_distinction(self):
        ...
        # The documented internal build version must match pyproject metadata.
        version_match = re.search(r'(?m)^version\s*=\s*"([^"]+)"', self.project_text)
        self.assertIsNotNone(version_match)
        self.assertIn(version_match.group(1), self.text)

    def test_only_portable_assets_are_documented(self):
        self.assertIn("当前版本只提供便携 ZIP", self.text)
        self.assertNotIn("RemoteMicRC003Setup-", self.text)
...
class PortableFlowContractTests(unittest.TestCase):
    ...
    def setUp(self):
        self.text = _PORTABLE_README_PATH.read_text(encoding="utf-8")
    ...
    def test_portable_stop_is_via_task_manager_not_a_stop_script(self):
        ...
        self.assertIn("停止时", self.normalized)

    def test_portable_removal_is_deleting_the_extracted_folder(self):
        self.assertIn("删除整个解压文件夹", self.normalized)
        self.assertIn("便携版", self.normalized)
...
class ConfigLogResidueDisclosureContractTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.portable_readme_text = _normalize_whitespace(
    ...
    def test_portable_removal_does_not_claim_runtime_directory_is_removed(self):
        for text in self.both:
            self.assertNotIn("不留系统级文件", text)
        self.assertIn("删除整个解压文件夹", self.portable_readme_text)
    ...
    def test_both_docs_offer_conditional_manual_cleanup(self):
        # The shared runtime directory must remain when another RC003 setup
        # still uses it.
        for text in self.both:
    ...
    def test_portable_removal_step_names_config_root_not_just_the_extracted_folder(self):
        ...
        portable_section_start = self.readme_text.index("#### 便携版使用流程")
        portable_section = self.readme_text[portable_section_start:]
        self.assertIn("配置、按键绑定和日志会继续保留", portable_section)
        self.assertIn(config.CONFIG_FILENAME, portable_section)


class WindowsPrereleaseAssetScopeContractTests(unittest.TestCase):
    """The two-file claim is scoped to the Windows RC003 candidate."""

    def setUp(self):
        self.text = _README_PATH.read_text(encoding="utf-8")

    def test_asset_count_claim_is_scoped_to_the_windows_candidate(self):
        self.assertIn("每个 RC003 Windows 候选预发行版提供以下两个文件", self.text)
        self.assertNotIn("每个预发行版提供以下两个文件", self.text)

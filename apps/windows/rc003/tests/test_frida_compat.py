...
class GadgetScriptLifecycleTests(unittest.TestCase):
    def test_keeps_socket_connection_alive_until_disconnect(self):
        script = frida_compat.frida_hid_tap_runtime.GADGET_SCRIPT
        self.assertIn("let connection = null;", script)
        self.assertIn("connection = currentConnection;", script)
        self.assertIn("connection = null;", script)

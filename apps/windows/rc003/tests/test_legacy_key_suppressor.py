...
class LegacyKeySuppressorDecisionTests(unittest.TestCase):
    ...
    def test_physicalizes_marked_ctrl_win_events_for_global_voice_shortcuts(self):
        gate = suppressor.LegacyKeySuppressor({0x74})
        for vk_code, scan_code in ((0xA2, 0x1D), (0x5B, 0x5B)):
            event = suppressor.KBDLLHOOKSTRUCT(
                vkCode=vk_code,
                scanCode=scan_code,
    ...
    def test_does_not_physicalize_unmarked_or_other_injected_events(self):
        gate = suppressor.LegacyKeySuppressor({0x74})
        for vk_code, extra_info in ((0xA5, 0), (0xA4, 0)):
            event = suppressor.KBDLLHOOKSTRUCT(
                vkCode=vk_code,
                scanCode=0x38,

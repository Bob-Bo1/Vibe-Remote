...
from ovb_rc003 import app, config, device_catalog, full_keys_setup, single_instance, windows_diagnostics
...
class FullKeysModeRoutingTests(_ArgvRestoringTestCase):
    def test_enable_full_keys_configures_then_starts_the_bridge(self):
        result = full_keys_setup.SetupResult(
            ok=True,
            message="configured",
            runtime_directory=Path("C:/runtime"),
        )
        sys.argv = ["ovb_rc003", "--enable-full-keys"]
        with (
            mock.patch.object(full_keys_setup, "enable_full_keys", return_value=result) as enable,
            mock.patch.object(full_keys_setup, "show_result") as show,
            mock.patch.object(main_module, "_run_bridge") as bridge,
        ):
            main_module.main()
    ...
    def test_disable_full_keys_returns_setup_exit_code(self):
        result = full_keys_setup.SetupResult(
            ok=True,
            message="disabled",
            runtime_directory=Path("C:/runtime"),
        )
        sys.argv = ["ovb_rc003", "--disable-full-keys"]
        with (
            mock.patch.object(full_keys_setup, "disable_full_keys", return_value=result) as disable,
            mock.patch.object(full_keys_setup, "show_result") as show,
        ):
            with self.assertRaises(SystemExit) as ctx:
                main_module.main()

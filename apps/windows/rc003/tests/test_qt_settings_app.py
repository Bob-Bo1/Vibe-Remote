...
class SettingsControllerTests(unittest.TestCase):
    ...
    def test_voice_app_defaults_to_wechat_hold_preset(self):
        controller, _ = self._make_controller()
        self.assertEqual(
            controller.voiceAppIndex,
            controller._VOICE_APP_ORDER.index(key_mapping.VOICE_APP_WECHAT),
        )
        self.assertEqual(controller.voiceHotkeyDisplay, "Ctrl+Win")
        self.assertEqual(controller.hotkeyText, "lctrl+lwin")
        self.assertEqual(controller.triggerModeIndex, 1)

    def test_selecting_doubao_applies_its_toggle_preset(self):
        controller, _ = self._make_controller()
        controller.voiceAppIndex = controller._VOICE_APP_ORDER.index(
        ...
        self.assertEqual(controller.voiceHotkeyDisplay, "右 Alt + Space")

    def test_selecting_custom_keeps_the_custom_editor_available(self):
        controller, _ = self._make_controller()
        controller.voiceAppIndex = controller._VOICE_APP_ORDER.index(
            key_mapping.VOICE_APP_CUSTOM
        )
        controller.hotkeyText = "shift+space"
        self.assertTrue(controller.voiceAppIsCustom)
        self.assertEqual(controller.hotkeyText, "shift+space")

    def test_save_settings_persists_voice_app_target(self):
        controller, _ = self._make_controller()
        controller.voiceAppIndex = controller._VOICE_APP_ORDER.index(
            key_mapping.VOICE_APP_DOUBAO
        )
        ...
        self.assertEqual(stored["voice_input_target"], key_mapping.VOICE_APP_DOUBAO)
        self.assertEqual(stored["voice_hotkey"], "ralt+space")
    ...
    def test_save_and_launch_launches_and_reports_started_when_save_succeeds(self):
        controller, _ = self._make_controller()
        controller._config["hid_report_tap_enabled"] = True
    ...
    def test_save_and_launch_prepares_full_keys_before_starting_bridge(self):
        controller, _ = self._make_controller()
        result = full_keys_setup.SetupResult(
            ok=False,
            message="已取消",
            runtime_directory=Path("D:/RemoteMic"),
            cancelled=True,
        )
        fake_launch_result = bridge_launcher.LaunchResult(
        ...
        with (
            mock.patch.object(
                full_keys_setup, "enable_full_keys", return_value=result
            ) as prepare,
            mock.patch.object(
                bridge_launcher, "launch_bridge", return_value=fake_launch_result
            ) as launch,
        ):
            controller.saveAndLaunch()
        prepare.assert_called_once_with(confirm=True)
        launch.assert_called_once()
        self.assertIn("基础按键", controller.fullKeysStatus)
        self.assertEqual(controller.connectionState, "waiting")
    ...
    def test_restore_defaults_resets_hotkey_and_trigger_mode(self):
        ...
        self.assertEqual(controller.triggerModeIndex, 1)
        self.assertEqual(controller.voiceAppIndex, controller._VOICE_APP_ORDER.index(
            key_mapping.VOICE_APP_WECHAT
        ))
...
def _find_mapping_row_combo(mapping_list, button_id, model):
    ...
    # The reference-style card keeps each gesture editor inside its own
    # labelled cell, so the editable ComboBox is deeper than the old flat
    # row's two-level child walk. Search the real QObject tree recursively;
    # this still returns the production ComboBox and does not bypass QML.
    def find_descendant(item):
        for child in item.children():
            if child.objectName() == "actionCombo_" + button_id:
                return child
            found = find_descendant(child)
            if found is not None:
                return found
        return None

    return find_descendant(current_item)
...
def _find(root, name):
    for child in (root.childItems() if hasattr(root, "childItems") else root.children()):
        if child.objectName() == name:
            return child
...
for object_name in ("connectionTabButton", "openLogButton", "hotkeyField"):
    ...
    if item is None:
        ...
        sys.exit(1)
    margin = 3
    corner = item.mapToScene(QPointF(7, item.property("height") / 2))
    ...
    for x in range(x0, x0 + width):
        for y in range(y0, y0 + height):
            ...
            if luminance < darkest:
                darkest = luminance
    results[object_name] = {"background": background_luminance, "darkest": darkest, "lightest": lightest}
...
class RenderedContrastTests(unittest.TestCase):
    ...
    def test_tab_button_plain_button_and_text_field_are_all_readable(self):
        ...
        self.assertEqual(
            result.returncode, 0, f"contrast probe subprocess failed: {result.stdout} {result.stderr}"
        )
        ...
        for object_name, label in self._LABELS.items():
            ...
            ratio = max(_contrast_ratio(measurement["background"], measurement["darkest"]),
                        _contrast_ratio(measurement["background"], measurement["lightest"]))
...
find_visual(window.contentItem(), "tabBar").setProperty("currentIndex", 1)
for _ in range(10):
    window.grabWindow()
    app.processEvents()

mapping_list = find_visual(window.contentItem(), "mappingList")
mapping_list.setProperty("currentIndex", model.index_of("power"))
current_card = mapping_list.property("currentItem")
combo = find_visual(current_card, "actionCombo_power")
center = combo.mapToScene(
    QPointF(combo.width() - 8, combo.height() / 2)
).toPoint()
QTest.mouseClick(window, Qt.LeftButton, Qt.NoModifier, center)
for _ in range(10):
    window.grabWindow()
...
popup_list = find_visual(window.contentItem(), "actionCombo_power_popupList")
assert popup_list is not None, "the real ComboBox popup ListView did not open"
content_height = popup_list.property("contentHeight")
viewport_height = popup_list.property("height")
assert popup_list.property("width") >= 220, "popup list is still too narrow"
assert viewport_height <= 300, "popup list is not height-limited"
assert content_height > viewport_height, "popup list has no scrollable overflow"
assert popup_list.property("interactive"), "popup ListView is not interactive"
print("OK")
"""


@unittest.skipUnless(_HAS_PYSIDE6, _SKIP_REASON)
class ButtonsPageComboPopupTests(unittest.TestCase):
    """XRBM-036: the real mapping ComboBox popup stays readable and gains a
    bounded, interactive ListView when the preset list is longer than the
    available screen height.
    """

    def test_mapping_combo_popup_is_wide_and_scrollable(self):
        import subprocess
        ...
        env["LOCALAPPDATA"] = tempfile.mkdtemp()
        result = subprocess.run(
            [sys.executable, "-c", _COMBO_POPUP_PROBE_SCRIPT],
            env=env,
            capture_output=True,
            text=True,
            timeout=120,
        ...
        self.assertEqual(
            result.returncode,
            0,
            f"ComboBox popup probe failed: {result.stdout}\n{result.stderr}",
        )
        self.assertIn("OK", result.stdout)

...
class _AppWiringTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # These wiring tests exercise the historical toggle path explicitly.
        # The product default is now the WeChat desktop preset: hold Ctrl+Win.
    ...
    def _wait_for_audio_start_fallback(self):
        time.sleep(app_module._VOICE_AUDIO_START_GRACE_SECONDS + 0.04)

    def tearDown(self):
        with self.app._voice_trigger_lock:
            self.app._cancel_voice_audio_start_timer_locked()
            self.app._cancel_voice_audio_stop_release_timer_locked()
            self.app._voice_audio_start_fallback_pending = False
...
class ButtonPathStartupOrderTests(_AppWiringTestCase):
    """The ordinary HID buttons must come up before BLE/ATVV connects.

    RC003 can still be visible to Windows while its BLE handshake is
    unavailable.  This test exercises the real ``_connect_once`` seam and
    locks the ordering that keeps Return/volume/navigation usable during a
    reconnect attempt.
    """

    def test_hid_paths_start_before_ble_discovery_and_handshake(self):
        events = []
        ...
        originals = (
            ...
            self.app._prewarm_voice_playback,
            app_module.ble_transport_winrt.discover_candidates,
            app_module.identity.select_single_candidate,
            app_module.ble_transport_winrt.RC003BleSession,
        )
        self.app._start_hid_listener = lambda: events.append("hid")
        self.app._start_hid_report_tap = lambda: events.append("tap")
        self.app._prewarm_voice_playback = lambda: events.append("prewarm")
        app_module.ble_transport_winrt.discover_candidates = _discover_candidates
        app_module.identity.select_single_candidate = lambda candidates: candidates[0]
        app_module.ble_transport_winrt.RC003BleSession = lambda **_kwargs: _FakeSession()
        try:
            _run(self.app._connect_once())
        finally:
            (
                self.app._start_hid_listener,
                self.app._start_hid_report_tap,
        ...
        self.assertEqual(events, ["hid", "tap", "discover", "ble", "prewarm"])


class HidReportTapStartupPolicyTests(_AppWiringTestCase):
    """The optional DLL-injection path must not run in the default package."""

    def test_disabled_tap_leaves_raw_input_path_as_the_only_button_path(self):
        class _UnexpectedTap:
            def __init__(self, *_args, **_kwargs):
                raise AssertionError("disabled HID tap must not be constructed")

        original = app_module.frida_compat.RC003HidReportTap
        self.app._config["hid_report_tap_enabled"] = False
        app_module.frida_compat.RC003HidReportTap = _UnexpectedTap
        try:
            self.app._start_hid_report_tap()
        finally:
            app_module.frida_compat.RC003HidReportTap = original
...
class HostHotkeyFailureSuppressesMicOpenTests(_AppWiringTestCase):
    ...
    def test_hold_physical_f5_up_releases_host_shortcut_immediately(self):
        ...
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        original_up = win32_input.send_wechat_key_combo_up
        win32_input.send_wechat_key_combo_down = lambda tokens: calls.append(("down", tokens))
        win32_input.send_wechat_key_combo_up = lambda tokens: calls.append(("up", tokens))
        try:
            self.app._on_legacy_key_event(0x74, True)
            self.app._on_legacy_key_event(0x74, False)
        finally:
            win32_input.send_wechat_key_combo_down = original_down
            win32_input.send_wechat_key_combo_up = original_up
    ...
    def test_hold_audio_stop_defers_release_until_physical_f5_up(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        ...
        try:
            ...
            self.assertTrue(self.app._voice.active)
            self.assertEqual(calls, [("down", ("lctrl", "lwin"))])
            self.app._on_legacy_key_event(0x74, False)
        finally:
            win32_input.send_wechat_key_combo_down = original_down
            win32_input.send_wechat_key_combo_up = original_up
    ...
    def test_late_audio_start_after_physical_release_does_not_reopen_voice(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        ...
        try:
            ...
            self.app._on_control_event(AudioStarted(session_id=1))
            self.app._on_control_event(MicButtonPressed())
            self._wait_for_audio_start_fallback()
        finally:
            win32_input.send_wechat_key_combo_down = original_down
            win32_input.send_wechat_key_combo_up = original_up
    ...
    def test_custom_left_ctrl_win_does_not_use_right_alt_transform(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
    ...
    def test_custom_left_ctrl_win_is_sent_as_hold_down_and_up(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        ...
        try:
            self.assertTrue(
                self.app._apply_voice_action(app_module.voice_controller.VoiceHostAction.KEY_DOWN)
            )
            self.assertTrue(
                self.app._apply_voice_action(app_module.voice_controller.VoiceHostAction.KEY_UP)
            )
        finally:
            win32_input.send_wechat_key_combo_down = original_down
            win32_input.send_wechat_key_combo_up = original_up
    ...
    def test_wechat_ctrl_win_uses_standard_sendinput_voice_path(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        original_up = win32_input.send_wechat_key_combo_up
        original_voice_down = win32_input.send_voice_key_combo_down
        original_voice_up = win32_input.send_voice_key_combo_up
        win32_input.send_wechat_key_combo_down = lambda tokens: calls.append(("wechat-down", tokens))
        win32_input.send_wechat_key_combo_up = lambda tokens: calls.append(("wechat-up", tokens))
        win32_input.send_voice_key_combo_down = lambda tokens: calls.append(("legacy-down", tokens))
        win32_input.send_voice_key_combo_up = lambda tokens: calls.append(("legacy-up", tokens))
        try:
            self.assertTrue(
                self.app._apply_voice_action(app_module.voice_controller.VoiceHostAction.KEY_DOWN)
            )
            ...
            win32_input.send_wechat_key_combo_down = original_down
            win32_input.send_wechat_key_combo_up = original_up
            win32_input.send_voice_key_combo_down = original_voice_down
            win32_input.send_voice_key_combo_up = original_voice_up

        self.assertEqual(
            calls,
            [
                ("wechat-down", ("lctrl", "lwin")),
                ("wechat-up", ("lctrl", "lwin")),
            ],
        )
    ...
    def test_audio_started_can_trigger_voice_when_legacy_f5_is_suppressed(self):
        original = win32_input.send_voice_key_combo_tap
        win32_input.send_voice_key_combo_tap = lambda tokens, **kwargs: None
        try:
            self.app._on_control_event(AudioStarted(session_id=1))
            self._wait_for_audio_start_fallback()
        finally:
    ...
    def test_late_duplicate_during_the_same_audio_session_is_ignored(self):
        hotkey_calls = []
        original = win32_input.send_voice_key_combo_tap
        win32_input.send_voice_key_combo_tap = lambda tokens: hotkey_calls.append(tokens)
        try:
...
class DirectHidVoiceTriggerTests(_AppWiringTestCase):
    """The direct HID report owns only the RC003 mic release edge.

    On this device the HID tap's Mic press report can arrive on a blocking
    thread; the keyboard-class F5 edge is the low-latency trigger and ATVV
    provides a delayed fallback when F5 is unavailable.  Its release edge is
    safe to use and prevents the host Ctrl+Win shortcut from being held until
    the 250 ms safety timer.
    """

    def test_direct_hid_mic_is_ignored_and_legacy_f5_starts_voice(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        win32_input.send_wechat_key_combo_down = lambda tokens: calls.append(tokens)
        try:
            mic_report = (0x003E).to_bytes(2, "little") + b"\x00" * 4
    ...
    def test_direct_hid_mic_release_closes_hold_without_waiting_for_f5(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("lctrl+lwin")
        calls = []
        original_down = win32_input.send_wechat_key_combo_down
        original_up = win32_input.send_wechat_key_combo_up
        win32_input.send_wechat_key_combo_down = lambda tokens: calls.append(
            ("down", tuple(tokens))
        )
        win32_input.send_wechat_key_combo_up = lambda tokens: calls.append(
            ("up", tuple(tokens))
        )
        try:
            self.app._on_legacy_key_event(0x74, True)
            mic_report = (0x003E).to_bytes(2, "little") + b"\x00" * 4
            empty_report = b"\x00" * 6
            self.app._on_direct_hid_report(1, mic_report)
            self.app._on_direct_hid_report(1, empty_report)
        finally:
            win32_input.send_wechat_key_combo_down = original_down
            win32_input.send_wechat_key_combo_up = original_up
...
class EventLoopOwnershipRegressionTests(unittest.TestCase):
    ...
    def test_unowned_default_loop_pattern_reproduces_the_exact_red_evidence(self):
        ...
        script = (
            "import asyncio, gc\n"
            "# Python 3.14 no longer creates the implicit loop on demand; seed\n"
            "# the same legacy ambient loop that older Python versions used.\n"
